...
/**
 * Verify EVVMNativeX402Adapter on Story Aeneid (StoryScan).
 */
async function main() {
  ...
  const deployment = JSON.parse(
    fs.readFileSync("deployments/bridge-evvm-native-adapter-latest.json", "utf8")
  );
  const args = deployment.constructorArgs;
  const constructorArgs = [args.token, args.evvmCore, args.evvmId, args.owner];

  console.log("🔍 Verifying EVVMNativeX402Adapter on StoryScan...");
