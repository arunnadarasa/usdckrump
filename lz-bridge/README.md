...
## 🌐 Deploy bridge relayer to Fly.io (24/7)

The **custom bridge** (BridgeVault on Base Sepolia → BridgeReceiver on Story Aeneid) uses a relayer that watches for `LockRequest` and calls `fulfillLock`. To run it 24/7 on [Fly.io](https://fly.io):

- **Guide:** [docs/FLY_DEPLOY.md](docs/FLY_DEPLOY.md)
- **Quick steps:** Install Fly CLI → `flyctl auth login` → set `BRIDGE_ATTESTER_KEY` and `BASE_SEPOLIA_RPC` secrets → `flyctl deploy`
